...
onMounted(async () => {
  ...
  try {
    const { data: config } = await axious.get(`${endpoint}/config`, { headers });
    configStore.update(config);

    const { data: certs } = await axious.get(`${endpoint}/keyring`, { headers });
    certsStore.update(certs);

    const { data } = await axious.get(`${endpoint}/ports`, { headers });
    portsStore.updateTable(data);

    for (const port of data) {
      axious.get(`${endpoint}/ports/${port.id}/status`, { headers }).then(({ data }) => {
