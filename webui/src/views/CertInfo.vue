<template>
    <v-table>
        <tbody>
            <tr>
                <td>{{ $t('certs.info.san') }}</td>
                <td>{{ info.san.join(', ') }}</td>
            </tr>
            <tr>
                <td>{{ $t('certs.info.fingerprint') }}</td>
                <td>{{ info.fingerprint }}</td>
            </tr>
            <tr>
                <td>{{ $t('certs.info.issuer') }}</td>
                <td>{{ info.issuer }}</td>
            </tr>
            <tr v-if="info.root_cert">
                <td>{{ $t('certs.info.root_cert') }}</td>
                <td>{{ info.root_cert }}</td>
            </tr>
            <tr>
                <td>{{ $t('certs.info.not_before') }}</td>
                <td>{{ (new Date(info.not_before * 1000)).toISOString() }}</td>
            </tr>
            <tr>
                <td>{{ $t('certs.info.not_after') }}</td>
    ...
    <v-card-actions class="justify-end">
        <v-btn color="red" @click="deleteDialog = true">
            {{ $t('certs.delete_cert.delete_cert') }}
        </v-btn>
    </v-card-actions>
    <v-dialog v-model="deleteDialog" width="auto">
        <v-card :title="$t('certs.delete_cert.delete_cert')">
            ...
            <v-card-actions class="justify-end">
                ...
                <v-btn color="red" @click="deleteCert">{{ $t('certs.delete_cert.delete') }}</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
